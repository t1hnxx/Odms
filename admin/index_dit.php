...
<div class="container-xxl bd-gutter mt-3 my-md-4 bd-layout">
	...
	<a class="btn border-0 m-1 text-decoration-underline fw-bolder" href="index.php" style="border: 1px solid #ec6d18;font-size: 100%;color:#ec6d18"><i class="bi bi-arrow-left-circle"></i>&nbspBACK</a>   
		<div style="color:#ec6d18" class="search-me">	
			...
			<h3><b>Department of Information Technology (DIT)</b></h3> 
		...
		<div class="col-sm-4 mx-auto">
					<?php					
					$dceaqueryall = "SELECT documentType, COUNT(*) as total_docu FROM users_documents WHERE  facultyDepartment = 'DIT' AND (documentType='thesis' OR documentType='journal' OR documentType='completion_form')";
					$resultsall = mysqli_query($conn,$dceaqueryall);
					$dcearowall = mysqli_fetch_assoc($resultsall);
					
					$dceaquery = "SELECT documentType, COUNT(*) as total_thesis1 FROM users_documents WHERE documentType='thesis' AND facultyDepartment = 'DIT'";
					$results = mysqli_query($conn,$dceaquery);
					$dcea1 = mysqli_fetch_assoc($results);

					$dceaojt = "SELECT documentType, COUNT(*) as total_journal2 FROM users_documents WHERE documentType='journal' AND facultyDepartment = 'DIT'";
					$resultdceaojt = mysqli_query($conn,$dceaojt);
					$dcea2 = mysqli_fetch_assoc($resultdceaojt);

					$dceacompletion = "SELECT documentType, COUNT(*) as total_completion3 FROM users_documents WHERE documentType='completion_form' AND facultyDepartment = 'DIT'";
    ...
    <div class="chart-body lh-1">
        ...
        <canvas id="dceaPieChart" width="300" height="200"></canvas>
                   	...
                   	<div class="col-sm-5 mx-auto">
						<?php					
						$ditStud = "SELECT studentCourse, COUNT(*) as total_ditStud FROM student_info WHERE userType='Student'  AND (studentCourse = 'BSIT' OR studentCourse = 'BSCS')";
						$resultditStud = mysqli_query($conn,$ditStud);
						$rowsditStud = mysqli_fetch_assoc($resultditStud);
						
						$ditBSIT = "SELECT studentCourse, COUNT(*) as total_BSIT FROM student_info WHERE userType='Student' AND studentCourse = 'BSIT'";
						$resultditBSIT = mysqli_query($conn,$ditBSIT);
						$rowsditBSIT = mysqli_fetch_assoc($resultditBSIT);

						$ditBSCS = "SELECT studentCourse, COUNT(*) as total_BSCS FROM student_info WHERE userType='Student' AND studentCourse = 'BSCS'";
						$resultditBSCS = mysqli_query($conn,$ditBSCS);
						$rowsditBSCS = mysqli_fetch_assoc($resultditBSCS);


						?>
            <div class="chart border-dark rounded-0">
			<div class="card-header border-dark border-2 fw-bolder text-center" style="font-size: 16px;">Total Students in the Department of Information Technology</div>
    <div class="card-body lh-1">
	<h3 class="card-title" style="color:#ec6d18; font-size: 40px; text-align: center"><?php echo $rowsditStud['total_ditStud']; ?> </h3>
        <!-- Add a canvas element for the bar graph for student distribution in DIT department -->
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Get the data from PHP variables for student distribution in DIT department
        var totalBSIT = <?php echo $rowsditBSIT['total_BSIT']; ?>;
        var totalBSCS = <?php echo $rowsditBSCS['total_BSCS']; ?>;
        // Create the bar graph for student distribution in DIT department
        var ctxDCEA4 = document.getElementById('barChartDCEA4').getContext('2d');
        var myBarChartDCEA4 = new Chart(ctxDCEA4, {
            type: 'bar',
            data: {
                labels: ['BS Information Technology', 'BS Computer Science'],
                datasets: [{
                    label: ['Total Number of Students'],
                    data: [totalBSIT, totalBSCS],
                    backgroundColor: [
