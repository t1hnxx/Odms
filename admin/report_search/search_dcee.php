<?php
    ...
    if(isset($_POST['query'])){
        ...
        $stmt = $conn->prepare("SELECT * FROM users_documents WHERE (documentName LIKE CONCAT('%',?,'%') OR documentID LIKE CONCAT('%',?,'%') OR documentStatus LIKE CONCAT('%',?,'%') OR submittedTo LIKE CONCAT('%',?,'%')OR facultyDepartment LIKE CONCAT('%',?,'%'))AND facultyDepartment = 'DCEE'");
        $stmt->bind_param("sssss",$search,$search, $search, $search, $search);

        
    }
    else{
        $stmt = mysqli_prepare($conn, "SELECT documentName, documentID, documentType, documentStatus, submittedTo FROM users_documents WHERE facultyDepartment = 'DCEE'");
    }
    $stmt->execute();
