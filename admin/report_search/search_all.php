<?php
    ...
    if(isset($_POST['query'])){
        ...
        $stmt = $conn->prepare("SELECT * FROM users_documents WHERE documentName LIKE CONCAT('%',?,'%') OR documentID LIKE CONCAT('%',?,'%') OR documentStatus LIKE CONCAT('%',?,'%') OR submittedTo LIKE CONCAT('%',?,'%') OR facultyDepartment LIKE CONCAT('%',?,'%')");
        $stmt->bind_param("sssss",$search, $search, $search, $search, $search);

        
    }
    else{
        $stmt = mysqli_prepare($conn, "SELECT documentName, documentID, documentType, documentStatus, submittedTo FROM users_documents");
    }
    $stmt->execute();
    $result = $stmt->get_result();

    if($result->num_rows > 0){
        $output = " <thead class='fw-bolder table-dark align-middle'>
        <tr>
        <td>Date Created</td>
