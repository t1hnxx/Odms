...
<div class="container-xxl bd-gutter mt-3 my-md-4 bd-layout">
	...
	<a class="btn border-0 m-1 text-decoration-underline fw-bolder" href="index.php" style="border: 1px solid #ec6d18;font-size: 100%;color:#ec6d18"><i class="bi bi-arrow-left-circle"></i>&nbspBACK</a>   
		<div style="color:#ec6d18" class="search-me">	
			...
			<h3><b>Department of Computer and Electronics Engineering (DCEE)</b></h3> 
        ...
        <div class="col-sm-4 mx-auto">
					<?php					
					$dceaqueryall = "SELECT documentType, COUNT(*) as total_docu FROM users_documents WHERE  facultyDepartment = 'DCEE' AND (documentType='thesis' OR documentType='journal' OR documentType='completion_form')";
					$resultsall = mysqli_query($conn,$dceaqueryall);
					$dcearowall = mysqli_fetch_assoc($resultsall);
					
					$dceaquery = "SELECT documentType, COUNT(*) as total_thesis1 FROM users_documents WHERE documentType='thesis' AND facultyDepartment = 'DCEE'";
					$results = mysqli_query($conn,$dceaquery);
					$dcea1 = mysqli_fetch_assoc($results);

					$dceaojt = "SELECT documentType, COUNT(*) as total_journal2 FROM users_documents WHERE documentType='journal' AND facultyDepartment = 'DCEE'";
					$resultdceaojt = mysqli_query($conn,$dceaojt);
					$dcea2 = mysqli_fetch_assoc($resultdceaojt);

					$dceacompletion = "SELECT documentType, COUNT(*) as total_completion3 FROM users_documents WHERE documentType='completion_form' AND facultyDepartment = 'DCEE'";
    ...
    <div class="chart-body lh-1">
        ...
        <canvas id="dceaPieChart" width="300" height="200"></canvas>
					...
					<div class="col-sm-5 mx-auto">
						<?php					
						$dceaStud = "SELECT COUNT(*) as total_dceaStud FROM student_info WHERE userType='Student' AND (studentCourse = 'BSCpE' OR studentCourse = 'BSECE' OR studentCourse = 'BSECE')";
						$resultdceaStud = mysqli_query($conn,$dceaStud);
						$rowsdceaStud = mysqli_fetch_assoc($resultdceaStud);
						
						$dceaBSCpE = "SELECT studentCourse, COUNT(*) as total_BSCpE FROM student_info WHERE userType='Student' AND studentCourse = 'BSCpE'";
						$resultdceaBSCpE = mysqli_query($conn,$dceaBSCpE);
						$rowsdceaBSCpE = mysqli_fetch_assoc($resultdceaBSCpE);

						$dceaBSECE = "SELECT studentCourse, COUNT(*) as total_BSECE FROM student_info WHERE userType='Student' AND studentCourse = 'BSECE'";
						$resultdceaBSECE = mysqli_query($conn,$dceaBSECE);
						$rowsdceaBSECE = mysqli_fetch_assoc($resultdceaBSECE);

						$dceaBSEE = "SELECT studentCourse, COUNT(*) as total_BSEE FROM student_info WHERE userType='Student' AND studentCourse = 'BSECE'";
						$resultdceaBSEE = mysqli_query($conn,$dceaBSEE);
						$rowsdceaBSEE = mysqli_fetch_assoc($resultdceaBSEE);
						?>
            <div class="chart border-dark rounded-0">
			<div class="card-header border-dark border-2 fw-bolder text-center" style="font-size: 16px;">Total Students in the Department of Computer and Electronics Engineering</div>
    <div class="card-body lh-1">
	<h3 class="card-title" style="color:#ec6d18; font-size: 40px; text-align: center"><?php echo $rowsdceaStud['total_dceaStud']; ?> </h3>
        <!-- Add a canvas element for the bar graph for student distribution in DCEE department -->
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Get the data from PHP variables for student distribution in DCEE department
        var totalBSCpE = <?php echo $rowsdceaBSCpE['total_BSCpE']; ?>;
        var totalBSECE = <?php echo $rowsdceaBSECE['total_BSECE']; ?>;
        var totalBSEE = <?php echo $rowsdceaBSEE['total_BSEE']; ?>;

        // Create the bar graph for student distribution in DCEE department
        var ctxDCEA4 = document.getElementById('barChartDCEA4').getContext('2d');
        var myBarChartDCEA4 = new Chart(ctxDCEA4, {
            type: 'bar',
            data: {
                labels: ['BS Computer Engineering', 'BS Electronics Engineering', 'BS Electrical Engineering'],
                datasets: [{
                    label: 'Total Number of Students',
                    data: [totalBSCpE, totalBSECE, totalBSEE],
                    backgroundColor: [
