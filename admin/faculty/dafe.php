<?php 
$query = "SELECT * FROM faculty_info WHERE facultyDepartment = 'DAFE'";
$result = mysqli_query($conn,$query); 

?>
<div>
  <button onclick="printFdafe()" class="btn btn-flat btn-success float-start" id="prints"><i class="bi bi-printer"></i>&nbsp Print</button>
</div>
<br> <br>
<div class="container text-white" id="areafdafe">
  <table class='table table-responsive-sm  table-striped table-bordered border-dark' id="fdafe">
...
function printFdafe() {
  var _el = $('<div>')
            ...
            _el.append('<div class="d-flex flex-row container-fluid">'+
                      ...
                      p.find('#photo').hide()
            p.find('#fdafe').removeClass("table-striped").addClass("border-black")
...
<script>
  ...
  w = document.getElementById("fdafe");
  v = w.getElementsByTagName("tbody");
	...
	for (i = 0; i < x.length; i++) {  
        if (!x[i].innerHTML.toLowerCase().includes(input)) { 
            ...
            head.innerHTML.style.display="list-item";
            x[i].style.display="";
