...
include("../dbcon.php"); 
	...
	if(isset($_POST['request'])){
		...
		$query1 = "SELECT * FROM users_documents WHERE documentType = '$request' AND facultyDepartment = 'DAFE'";
		$result1 = mysqli_query($conn, $query1);
		$count = mysqli_num_rows($result1);
	
    if($count > 0){
      $output = " <thead class='fw-bolder table-dark align-middle' id='headafe'>
      <tr>
      <td>Date Created</td>
 ...
 while( $row = mysqli_fetch_assoc($result1)){
  ...
  $name = [];
    ...
    if($row['submittedTo'] == "None"){
       ...
       $submittedTo = $row['facultyFname'] . ' ' . $row['facultyMname'] . ' ' . $row['facultyFname']; 
     }
