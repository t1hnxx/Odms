    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
        ...
        for ($i = 1; $i <= $sheetCount; $i ++) {
            ...
            if (isset($spreadSheetAry[$i][0])) {
                $facultyFname = mysqli_real_escape_string($conn, $spreadSheetAry[$i][0]);
            }
            $facultyMname = "";
            if (isset($spreadSheetAry[$i][1])) {
                $facultyMname = mysqli_real_escape_string($conn, $spreadSheetAry[$i][1]);
            }
			$facultyLname = "";
            if (isset($spreadSheetAry[$i][2])) {
                $facultyLname = mysqli_real_escape_string($conn, $spreadSheetAry[$i][2]);
            }
            $facultyGender = "";
            if (isset($spreadSheetAry[$i][3])) {
                $facultyGender = mysqli_real_escape_string($conn, $spreadSheetAry[$i][3]);
            }
			$facultyUsername = "";
            if (isset($spreadSheetAry[$i][4])) {
                $facultyUsername = mysqli_real_escape_string($conn, $spreadSheetAry[$i][4]);
            }
			$facultyEmail = "";
            if (isset($spreadSheetAry[$i][5])) {
                $facultyEmail = mysqli_real_escape_string($conn, $spreadSheetAry[$i][5]);
            }
			$facultyPassword = "";
            if (isset($spreadSheetAry[$i][6])) {
				
                $facultyPassword = mysqli_real_escape_string($conn, $spreadSheetAry[$i][6]);
            }
			$position = "";
            if (isset($spreadSheetAry[$i][7])) {
                $position = mysqli_real_escape_string($conn, $spreadSheetAry[$i][7]);
            }
			$facultyDepartment = "";
            if (isset($spreadSheetAry[$i][8])) {
                $facultyDepartment = mysqli_real_escape_string($conn, $spreadSheetAry[$i][8]);
            }
            ...
            if (! empty($facultyFname)  || ! empty($facultyLname) || ! empty($facultyGender) || ! empty($facultyDepartment) || ! empty($facultyEmail)){
				//for username
				$num = '1234567890';
                ...
                $query = "insert into faculty_email(facultyFname,facultyMname,facultyLname,facultyGender,facultyUsername,facultyEmail,facultyPassword,position,facultyDepartment) values(?,?,?,?,?,?,?,?,?)";
                $paramType = "sssssssss";
                $paramArray = array(
                    $facultyFname,
                    $facultyMname,
				...
				$query1 = "insert into faculty_sheets(facultyFname,facultyMname,facultyLname,facultyGender,facultyUsername,facultyEmail,facultyPassword,position,facultyDepartment) values(?,?,?,?,?,?,?,?,?)";
...
<br><br><br>
	...
	<div class="row">
	 ...
	 <div class="col-md-10 mt-1">
		...
		<a class="btn border-0 btn-md m-3 mb-1 text-decoration-underline fw-bolder" href="admin-faculty.php" style="border: 2px solid #ec6d18;font-size: 120%;color:#ec6d18"><i class="bi bi-arrow-left-circle"></i>&nbspBACK</a> 
				...
				<div Class="input-row">
					<label for="file" class="border border-black rounded-2 text-dark" style="cursor: pointer;">&nbsp<i class="bi bi-file-earmark-spreadsheet"></i>&nbsp INSERT YOUR FILE HERE.&nbsp</label>
						...
						<input type="file" name="file" id="file" class="file"
							...
							<br><div class="text-black" id="display-image"></div>
					<div class="import"> <br>
...
<?php
$sqlSelect = "SELECT * FROM faculty_email";
