...
<div class="container-xxl bd-gutter mt-3 my-md-4 bd-layout">
	...
	<a class="btn border-0 m-1 text-decoration-underline fw-bolder" href="index.php" style="border: 1px solid #ec6d18;font-size: 100%;color:#ec6d18"><i class="bi bi-arrow-left-circle"></i>&nbspBACK</a>   
		<div style="color:#ec6d18" class="search-me">	
			...
			<h3><b>Department of Agriculture and Food Engineering (DAFE)</b></h3> 
            <hr style="height:2px;border-width:0;color:black;background-color:black">
				...
				<div class="col-sm-4 mx-auto">
					<?php					
					$dceaqueryall = "SELECT documentType, COUNT(*) as total_docu FROM users_documents WHERE  facultyDepartment = 'DAFE' AND (documentType='thesis' OR documentType='journal' OR documentType='completion_form')";
					$resultsall = mysqli_query($conn,$dceaqueryall);
					$dcearowall = mysqli_fetch_assoc($resultsall);
					
					$dceaquery = "SELECT documentType, COUNT(*) as total_thesis1 FROM users_documents WHERE documentType='thesis' AND facultyDepartment = 'DAFE'";
					$results = mysqli_query($conn,$dceaquery);
					$dcea1 = mysqli_fetch_assoc($results);

					$dceaojt = "SELECT documentType, COUNT(*) as total_journal2 FROM users_documents WHERE documentType='journal' AND facultyDepartment = 'DAFE'";
					$resultdceaojt = mysqli_query($conn,$dceaojt);
					$dcea2 = mysqli_fetch_assoc($resultdceaojt);

					$dceacompletion = "SELECT documentType, COUNT(*) as total_completion3 FROM users_documents WHERE documentType='completion_form' AND facultyDepartment = 'DAFE'";
    ...
    <div class="chart-body lh-1">
        ...
        <canvas id="dceaPieChart" width="300" height="200"></canvas>
					...
					<div class="col-sm-5 mx-auto">
						<?php					
						$dafeStud = "SELECT studentCourse, COUNT(*) as total_dceaStud FROM student_info WHERE userType='Student' AND studentCourse = 'BSABE'";
						$resultdafeStud = mysqli_query($conn,$dafeStud);
						$rowsdafeStud = mysqli_fetch_assoc($resultdafeStud);
						
						$dafeBSABE = "SELECT studentCourse, COUNT(*) as total_BSABE FROM student_info WHERE userType='Student' AND studentCourse = 'BSABE'";
						$resultdafeBSABE = mysqli_query($conn,$dafeBSABE);
						$rowsdafeBSABE = mysqli_fetch_assoc($resultdafeBSABE);


						?>
            <div class="chart border-dark rounded-0">
			<div class="card-header border-dark border-2 fw-bolder text-center" style="font-size: 16px;">Total Students in the Department of Agriculture and Food Engineering</div>
    <div class="card-body lh-1">
	<h3 class="card-title" style="color:#ec6d18; font-size: 40px; text-align: center"><?php echo $rowsdafeStud['total_dceaStud']; ?> </h3>
        <!-- Add a canvas element for the bar graph for student distribution in DAFE department -->
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Get the data from PHP variables for student distribution in DAFE department
        var totalBSABE = <?php echo $rowsdafeBSABE['total_BSABE']; ?>;

        // Create the bar graph for student distribution in DAFE department
        var ctxDCEA4 = document.getElementById('barChartDCEA4').getContext('2d');
        var myBarChartDCEA4 = new Chart(ctxDCEA4, {
            type: 'bar',
            data: {
                labels: ['BSBS Agricultural and Biosystems Engineering'],
                datasets: [{
