...
<div class="container-xxl bd-gutter mt-3 my-md-4 bd-layout">
	...
	<a class="btn border-0 m-1 text-decoration-underline fw-bolder" href="index.php" style="border: 1px solid #ec6d18;font-size: 100%;color:#ec6d18"><i class="bi bi-arrow-left-circle"></i>&nbspBACK</a>   
		<div style="color:#ec6d18" class="search-me">	
			...
			<h3><b>Department of Industrial Engineering and Technology (DIET)</b></h3> 
		...
		<div class="col-sm-4 mx-auto">
					<?php					
					$dceaqueryall = "SELECT documentType, COUNT(*) as total_docu FROM users_documents WHERE  facultyDepartment = 'DIET' AND (documentType='thesis' OR documentType='journal' OR documentType='completion_form')";
					$resultsall = mysqli_query($conn,$dceaqueryall);
					$dcearowall = mysqli_fetch_assoc($resultsall);
					
					$dceaquery = "SELECT documentType, COUNT(*) as total_thesis1 FROM users_documents WHERE documentType='thesis' AND facultyDepartment = 'DIET'";
					$results = mysqli_query($conn,$dceaquery);
					$dcea1 = mysqli_fetch_assoc($results);

					$dceaojt = "SELECT documentType, COUNT(*) as total_journal2 FROM users_documents WHERE documentType='journal' AND facultyDepartment = 'DIET'";
					$resultdceaojt = mysqli_query($conn,$dceaojt);
					$dcea2 = mysqli_fetch_assoc($resultdceaojt);

					$dceacompletion = "SELECT documentType, COUNT(*) as total_completion3 FROM users_documents WHERE documentType='completion_form' AND facultyDepartment = 'DIET'";
    ...
    <div class="chart-body lh-1">
        ...
        <canvas id="dceaPieChart" width="300" height="200"></canvas>
					...
					<div class="col-sm-5 mx-auto">
						<?php					
						$dietStud = "SELECT COUNT(*) as total_dietStud FROM student_info WHERE userType='Student' AND (studentCourse = 'BSIndT-ET' OR studentCourse = 'BSIndT-ELEX' OR studentCourse = 'BSIndT-AT' OR studentCourse = 'BSIE')";
						$resultdietStud = mysqli_query($conn,$dietStud);
						$rowsdietStud = mysqli_fetch_assoc($resultdietStud);
						
						$dietBSINDT = "SELECT studentCourse, COUNT(*) as total_BSINDT FROM student_info WHERE userType='Student' AND studentCourse = 'BSIndT-ET'";
						$resultdietBSINDT = mysqli_query($conn,$dietBSINDT);
						$rowsdietBSINDT = mysqli_fetch_assoc($resultdietBSINDT);

						$dietBSINDT2 = "SELECT studentCourse, COUNT(*) as total_BSINDT2 FROM student_info WHERE userType='Student' AND studentCourse = 'BSIndT-ELEX'";
						$resultdietBSINDT2 = mysqli_query($conn,$dietBSINDT2);
						$rowsdietBSINDT2 = mysqli_fetch_assoc($resultdietBSINDT2);

						$dietBSINDT3 = "SELECT studentCourse, COUNT(*) as total_BSINDT3 FROM student_info WHERE userType='Student' AND studentCourse = 'BSIndT-AT'";
						$resultdietBSINDT3 = mysqli_query($conn,$dietBSINDT3);
						$rowsdietBSINDT3 = mysqli_fetch_assoc($resultdietBSINDT3);

						$dietBSIE = "SELECT studentCourse, COUNT(*) as total_BSIE FROM student_info WHERE userType='Student' AND studentCourse = 'BSIE'";
						$resultdietBSIE = mysqli_query($conn,$dietBSIE);
						$rowsdietBSIE = mysqli_fetch_assoc($resultdietBSIE);


						?>
            <div class="chart border-dark rounded-0">
			<div class="card-header border-dark border-2 fw-bolder text-center" style="font-size: 16px;">Total Students in the Department of Industrial Engineering and Technology</div>

    <div class="card-body lh-1">
	<h3 class="card-title" style="color:#ec6d18; font-size: 40px; text-align: center"><?php echo $rowsdietStud['total_dietStud']; ?> </h3>
        <!-- Add a canvas element for the bar graph for student distribution in DIET department -->
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Get the data from PHP variables for DIET departmen
        var totalSubmittedDCEA = <?php echo $dcearow1['total_submitted']; ?>;
        var totalOnHandDCEA = <?php echo $dcearow5['total_onhand']; ?>;
        var totalReviewDCEA = <?php echo $dcearow3['total_review']; ?>;
        var totalPickupDCEA = <?php echo $dcearow4['total_pickup']; ?>;

        // Create the pie chart for DIET department
        var ctxDCEA = document.getElementById('pieChartDCEA').getContext('2d');
        var myPieChartDCEA = new Chart(ctxDCEA, {
            type: 'pie',
            data: {
                labels: ['Submitted', 'On-hand', 'Under Review', 'Available for Pick-up'],
                datasets: [{
                    data: [totalSubmittedDCEA, totalOnHandDCEA, totalReviewDCEA, totalPickupDCEA],
                    backgroundColor: [
                        '#c2dfff',
                        '#fff494',
                        '#ffcd85',
...
<script>
    // JavaScript code for Chart.js
    document.addEventListener('DOMContentLoaded', function() {
        // Get the data from PHP variables for student distribution in DIET department
        var totalBSINDT = <?php echo $rowsdietBSINDT['total_BSINDT']; ?>;
		var totalBSINDT2 = <?php echo $rowsdietBSINDT2['total_BSINDT2']; ?>;
		var totalBSINDT3 = <?php echo $rowsdietBSINDT3['total_BSINDT3']; ?>;
        var totalBSIE = <?php echo $rowsdietBSIE['total_BSIE']; ?>;
        // Create the bar graph for student distribution in DIET department
        var ctxDCEA4 = document.getElementById('barChartDCEA4').getContext('2d');
        var myBarChartDCEA4 = new Chart(ctxDCEA4, {
            type: 'bar',
            data: {
                labels: ['BSIndT-ET','BSIndT-ELEX', 'BSIndT-AT', 'BSIE'],
                datasets: [{
                    label: 'Total Number of Students',
                    data: [totalBSINDT, totalBSINDT2, totalBSINDT3, totalBSIE],
                    backgroundColor: [
                        'rgba(236, 109, 24, 0.8)',
                        'rgba(255, 159, 64, 0.8)',
                        'rgba(255, 193, 7, 0.8)',
