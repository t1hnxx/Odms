<?php 
$query = "SELECT * FROM users_documents WHERE facultyDepartment = 'DCEE'";
...
<div class="d-flex flex-row" id="filter">
  <div class="col-sm-6">
    ...
    <input type="text" name="search" id="search_dcee" class="form-control rounded-0 w-100" placeholder="Search">
    <br>
    <span for="getdcee">Document Type: </span>
    <select name="getdcee" id="getdcee" class=" w-50 form-select form-select-sm" aria-label="Small select example">
  ...
  <div class="col-sm-4 ">
    <br>
    <button onclick="printdcee()" class="btn btn-flat btn-success float-end"><i class="bi bi-printer"></i>&nbsp Print</button>
  </div>
</div>

<br>
<div class="container text-white" id="areadcee">
  <table class='table table-responsive-sm table-striped table-bordered border-dark' id="dcee">
...
function printdcee() {
  var _el = $('<div>')
            ...
            p.find('#dcee').removeClass("table-striped").addClass("border-black")
...
<script>
 $(document).ready(function(){
  $("#getdcee").on('change', function(){
			var value = $(this).val();
			//alert(value);	
			$.ajax({
				url: "report_select/getdcee.php",
				type: "POST",	
				data: 'request=' + value,
				beforeSend:function(){
					$("#dcee").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
				},
  ...
  $("#search_dcee").keyup(function(){
    var search = $(this).val();
    $.ajax({
      url: 'report_search/search_dcee.php',
      method: 'post',
      data:{query:search},
      beforeSend:function(){
					$("#dcee").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
