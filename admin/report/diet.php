<?php 
$query = "SELECT * FROM users_documents WHERE facultyDepartment = 'DIET'";
...
<div class="d-flex flex-row" id="filter">
  <div class="col-sm-6">
    ...
    <input type="text" name="search" id="search_diet" class="form-control rounded-0 w-100" placeholder="Search">
    <br>
    <span for="getdiet">Document Type: </span>
    <select name="getdiet" id="getdiet" class=" w-50 form-select form-select-sm" aria-label="Small select example">
  ...
  <div class="col-sm-4 ">
    <br>
    <button onclick="printdiet()" class="btn btn-flat btn-success float-end"><i class="bi bi-printer"></i>&nbsp Print</button>
  </div>
</div>

<br>
<div class="container text-white" id="areadiet">
  <table class='table  table-responsive-sm table-striped table-bordered border-dark' id="ddiet">
    <thead class="fw-bolder table-dark align-middle" id="headiet">
              <tr>
                    <td>Date Created</td>
...
function printdiet() {
  var _el = $('<div>')
            ...
            _el.append('<div class="d-flex flex-row container-fluid">'+
                      ...
                      '<h6 class="text-center fw-bold" style="font-size: 16px">Department of Industrial Engineering and Technology</h6>' +
                      '</div>' +
            ...
            p.find('#diet').removeClass("table-striped").addClass("border-black")
            p.find('#headiet').removeClass("table-dark").addClass("border-black") 
            _el.append(p.html())
...
<script>
 $(document).ready(function(){
  $("#getdiet").on('change', function(){
			var value = $(this).val();
			//alert(value);	
			$.ajax({
				url: "report_select/getdiet.php",
				type: "POST",	
				data: 'request=' + value,
				beforeSend:function(){
					$("#diet").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
				},
  ...
  $("#search_diet").keyup(function(){
    var search = $(this).val();
    $.ajax({
      url: 'report_search/search_diet.php',
      method: 'post',
      data:{query:search},
      beforeSend:function(){
					$("#diet").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
