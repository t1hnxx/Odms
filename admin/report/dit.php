<?php 
$query = "SELECT * FROM users_documents WHERE facultyDepartment = 'DIT'";
...
<div class="d-flex flex-row" id="filter">
  <div class="col-sm-6">
    ...
    <input type="text" name="search" id="search_dit" class="form-control rounded-0 w-100" placeholder="Search">
    <br>
    <span for="getdit">Document Type: </span>
    <select name="getdit" id="getdit" class=" w-50 form-select form-select-sm" aria-label="Small select example">
  ...
  <div class="col-sm-4 ">
    <br>
    <button onclick="printdit()" class="btn btn-flat btn-success float-end"><i class="bi bi-printer"></i>&nbsp Print</button>
  </div>
</div>

<br>
<div class="container text-white" id="areadit">
  <table class='table table-responsive-sm table-striped table-bordered border-dark' id="dit">
    <thead class="fw-bolder table-dark align-middle" id="headit">
              <tr>
              <td>Date Created</td>
...
function printdit() {
  var _el = $('<div>')
            ...
            _el.append('<div class="d-flex flex-row container-fluid">'+
                      ...
                      '<div class="text-dark"><h4 class="text-center fw-bolder">&nbsp Cavite State University </h4>' +
                      '<h5 class="text-center" style="font-size: 20px">College of Engineering and Information Technology</h5>' +
                      '<h5 class="text-center fw-bold" style="font-size: 25px"><sub>Department of Information Technology</sub></h5>' +
                      '</div>' +
            ...
            p.find('#dit').removeClass("table-striped").addClass("border-black")
            p.find('#headit').removeClass("table-dark").addClass("border-black") 
            _el.append(p.html())
...
<script>
 $(document).ready(function(){
  $("#getdit").on('change', function(){
			var value = $(this).val();
			//alert(value);	
			$.ajax({
				url: "report_select/getdit.php",
				type: "POST",	
				data: 'request=' + value,
				beforeSend:function(){
					$("#dit").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
				},
  ...
  $("#search_dit").keyup(function(){
    var search = $(this).val();
    $.ajax({
      url: 'report_search/search_dit.php',
      method: 'post',
      data:{query:search},
      beforeSend:function(){
					$("#dit").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
