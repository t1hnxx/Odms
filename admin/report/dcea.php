<?php 
$query = "SELECT * FROM users_documents WHERE facultyDepartment = 'DCEA'";
...
<div class="d-flex flex-row" id="filter">
  <div class="col-sm-6">
    ...
    <input type="text" name="search" id="search_dcea" class="form-control rounded-0 w-100" placeholder="Search">
    <br>
    <span for="getdcea">Document Type: </span>
      <select name="getdcea" id="getdcea" class=" w-50 form-select form-select-sm" aria-label="Small select example">
  ...
  <div class="col-sm-4 ">
    <br>
    <button onclick="printdcea()" class="btn btn-flat btn-success float-end"><i class="bi bi-printer"></i>&nbsp Print</button>
  </div>
</div>

<br>
<div class="container text-white" id="areadcea">
  <table class='table  table-responsive-sm table-striped table-bordered border-dark' id="dcea">
...
function printdcea() {
  var _el = $('<div>')
            ...
            p.find('#dcea').removeClass("table-striped").addClass("border-black")
...
<script>
 $(document).ready(function(){
  $("#getdcea").on('change', function(){
			var value = $(this).val();
			//alert(value);	
			$.ajax({
				url: "report_select/getdcea.php",
				type: "POST",	
				data: 'request=' + value,
				beforeSend:function(){
					$("#dcea").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
				},
  ...
  $("#search_dcea").keyup(function(){
    var search = $(this).val();
    $.ajax({
      url: 'report_search/search_dcea.php',
      method: 'post',
      data:{query:search},
      beforeSend:function(){
					$("#dcea").html("<br><br><div class='d-flex justify-content-center'><img src='loading45.gif' height='70' width='70'></div>");
